...
const Footer = () => (
  <footer className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    ...
    <div className='w-full flex justify-between items-center flex-col md:flex-row pt-6 border-t-[1px] border-t-[#3F3E45]'>
      ...
      <div className='flex flex-row mt-6 sm:mt-0'>
        {socialMedia.map((icon, index) => (
          <img src={icon.icon} key={icon.id} alt={icon.id} className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length - 1 ? 'mr-6' : 'mr-0'}`} />
        ))}
      </div>
    </div>
  </footer>
